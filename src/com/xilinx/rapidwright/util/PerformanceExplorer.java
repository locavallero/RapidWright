...
public class PerformanceExplorer {
	...
	private static final double DEFAULT_MIN_CLK_UNCERT = 1.000;
	private static final double DEFAULT_MAX_CLK_UNCERT = 1.500;
	private static final double DEFAULT_STEP_CLK_UNCERT = 0.500;
	...
	public ArrayList<String> createTclScript(String initialDcp, String instDirectory,
		...
		lines.add("set_clock_uncertainty -setup "+ clockUncertainty.replace(",", ".") +" [get_clocks "+clkName+"]");
		if(pblock != null){
	...
	private static OptionParser createOptionParser(){
		...
		OptionParser p = new OptionParser() {{
			...
			accepts(MIN_CLK_UNCERTAINTY_OPT).withOptionalArg().defaultsTo(printNS(DEFAULT_MIN_CLK_UNCERT).replace(",", ".")).describedAs("Min clk uncertainty (ns)");
			accepts(MAX_CLK_UNCERTAINTY_OPT).withOptionalArg().defaultsTo(printNS(DEFAULT_MAX_CLK_UNCERT).replace(",", ".")).describedAs("Max clk uncertainty (ns)");
			accepts(CLK_UNCERTAINTY_STEP_OPT).withOptionalArg().defaultsTo(printNS(DEFAULT_STEP_CLK_UNCERT).replace(",", ".")).describedAs("Clk uncertainty step (ns)");
